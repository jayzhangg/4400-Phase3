...
BEGIN
    	...
    	SELECT companyName AS comName, COUNT(manager_name) AS numEmployee, COUNT(theater_city) AS numCityCover,  COUNT(theater_name) AS numTheater
    	FROM company 
		LEFT JOIN manager ON companyName = manager_works_in
    	LEFT JOIN theater on companyName = theater_owned_by
        WHERE ((companyName = manager_works_in) AND (companyName = theater_owned_by) AND (companyName = i_comName) ) OR 
(( i_comName = "ALL") AND (companyName = manager_works_in) AND (companyName = theater_owned_by))
		GROUP BY companyName
		HAVING  ((i_minCity IS NULL OR COUNT(DISTINCT(theater_city)) >= i_minCity)
     	AND (i_maxCity IS NULL OR COUNT(DISTINCT(theater_city)) <= i_maxCity)
    	AND (i_minTheater IS NULL OR COUNT(DISTINCT(theater_name)) >= i_minTheater)
     	AND (i_maxTheater IS NULL OR COUNT(DISTINCT(theater_name)) <= i_maxTheater)
     	AND (i_minEmployee IS NULL OR COUNT(DISTINCT(manager_name))  >= i_minEmployee)
    	AND (i_maxEmployee IS NULL OR COUNT(DISTINCT(manager_name))  <= i_maxEmployee))
